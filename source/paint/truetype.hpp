...
namespace nana::paint::detail
{
	...
	public:
		...
		{
			return (static_cast<std::uint32_t>(swap(std::uint16_t(val & 0xFFFF))) << 16) | swap(std::uint16_t(val >> 16));
		}
	private:
		void _m_parse(std::ifstream& ifs)
		{
			...
			const std::size_t num_of_tables = swap(offset_table.num_of_tables);
			for (std::size_t i = 0; i < num_of_tables; ++i)
			{
				...
				{
					const std::size_t directory_offset = swap(table_directory.offset);
					...
					const std::size_t name_records_count = swap(name_table.name_records_count);
					const std::size_t storage_offset = swap(name_table.storage_offset);
					...
					for (std::size_t u = 0; u < name_records_count; ++u)
					{
						...
						std::size_t string_length = swap(record.string_length);

						auto const filepos = ifs.tellg();
						ifs.seekg(directory_offset + swap(record.string_offset) + storage_offset, std::ios::beg);
	...
	class truetype_collection
	{
		struct ttc_header
		{
			...
			std::uint16_t major_version;
			std::uint16_t minor_version;
			std::uint32_t fonts_count;
		};
	public:
		truetype_collection(std::filesystem::path p):
			ifs_(p, std::ios::binary)
		{
			ttc_header header;
			ifs_.read(reinterpret_cast<char*>(&header), sizeof(header));

			if (ifs_.gcount() != sizeof(header))
				return;

			if (header.tag != *reinterpret_cast<const std::uint32_t*>("ttcf"))
				return;

			header.fonts_count = truetype::swap(header.fonts_count);

			offsets_.resize(header.fonts_count);

			ifs_.read(reinterpret_cast<char*>(offsets_.data()), sizeof(std::uint32_t) * header.fonts_count);
			for (auto& off : offsets_)
				off = truetype::swap(off);
		}
		...
		std::optional<truetype> read(std::size_t idx)
		{
			if (idx >= offsets_.size())
				return {};

			ifs_.seekg(offsets_[idx], std::ios::beg);
			return { ifs_ };
		}
	private:
		std::ifstream ifs_;
