...
namespace nana
{
	...
	namespace drawerbase::treebox
	{
...
#ifdef __cpp_char8_t
		item_proxy& item_proxy::icon(std::u8string_view id)
		{
			node_->value.second.img_idstr = to_string(id);
			return *this;
		}
#endif
...
#ifdef __cpp_char8_t
		item_proxy& item_proxy::key(std::u8string_view s)
		{
			return key(to_string(s));
		}
#endif
...
#ifdef __cpp_char8_t
		item_proxy& item_proxy::text(std::u8string_view id)
		{
		...
		bool item_proxy::operator==(std::wstring_view s) const
		{
		...
		{
			return (node_ && (node_->value.second.text == to_string(s)));
		}
#endif
...
#ifdef __cpp_char8_t
	treebox::node_image_type& treebox::icon(std::u8string_view id)
	{
		return get_drawer_trigger().icon(to_string(id));
	}

	void treebox::icon_erase(std::u8string_view id)
	{
		get_drawer_trigger().icon_erase(to_string(id));
	}		
#endif
...
#ifdef __cpp_char8_t
	treebox::item_proxy treebox::find(std::u8string_view keypath)
	{
		return find(to_string(keypath));
	}
#endif
...
#ifdef __cpp_char8_t
	std::u8string treebox::make_key_path(item_proxy i, std::u8string_view splitter) const
	{
		auto p = make_key_path(i, to_string(splitter));
		return to_u8str(p);
	}
#endif
