...
namespace nana
{
	...
	namespace drawerbase::listbox
	{
...
#ifdef __cpp_char8_t
			cell::cell(std::u8string_view text)
				: text(to_string(text))
			{}

			cell::cell(std::u8string_view text, const format& fmt)
				:	text(to_string(text)),
					custom_format(std::make_unique<format>( fmt ))
			{}
#endif
		...
		public:
				...
				: public column_interface
			{
				...
				std::function<bool(const std::string&,  std::any*,
						           const std::string&,  std::any*,        bool reverse)> weak_ordering;
			...
			public:
				...
				{
					caption = nana::detail::to_nstring(std::move(text_utf8));
					_m_refresh();
				}

#ifdef __cpp_char8_t
				void text(std::u8string_view text) override
				{
					caption = nana::detail::to_nstring(text);
					_m_refresh();
				}
#endif
		...
		struct item_data
		{
			...
			mutable std::unique_ptr<std::any> anyobj;
			...
			item_data(const item_data& r)
				:	cells(r.cells ? std::make_unique<container>(*r.cells) : nullptr),
					...
					img(r.img),
					flags(r.flags),
					anyobj(r.anyobj ? new std::any(*r.anyobj) : nullptr)
			{}
			...
			item_data& operator=(const item_data& r)
			{
				if (this != &r)
				{
					...
					flags = r.flags;
					anyobj.reset(r.anyobj ? new std::any(*r.anyobj) : nullptr);
					bgcolor = r.bgcolor;
					fgcolor = r.fgcolor;
					img = r.img;
				}
		...
		public:
			...
			std::function<std::function<bool(const std::string&, std::any*,
							const std::string&, std::any*, bool reverse)>(std::size_t) > fetch_ordering_comparer;
			...
			std::any * anyobj(const index_pair& id, bool allocate_if_empty) const
			{
		...
		struct essence
		{
			...
			essence()
			{
				...
				lister.fetch_ordering_comparer = [this](std::size_t pos) -> std::function<bool(const std::string&, std::any*, const std::string&, std::any*, bool reverse)>
				{
		...
		private:
			void _m_draw_categ(const category_t& categ, int x, int y, int txtoff, unsigned width, nana::color bgcolor, item_state state)
			{
				...
				if (categ.display_number)
				{
					...
					native_string_type str = nana::detail::to_nstring('(' + std::to_string(categ.items.size()) + ')');
		...
		//class item_proxy
...
#ifdef __cpp_char8_t
			item_proxy& item_proxy::text(size_type col, std::u8string_view str)
			{
				return text(col, to_string(str));
			}
#endif
...
#ifdef __cpp_char8_t
			bool item_proxy::operator==(std::u8string_view sv) const
			{
				return (text(0) == to_string(sv));
			}
#endif
			...
			std::any * item_proxy::_m_value(bool alloc_if_empty)
			{
		...
		//the member cat_ is used for fast accessing to the category
...
#ifdef __cpp_char8_t
			void cat_proxy::append(std::initializer_list<std::u8string> arg)
			{
				const auto items = columns();
				push_back(std::string{});
				item_proxy ip{ ess_, index_pair(pos_, size() - 1) };
			...
			{
				auto text = nana::detail::to_nstring(s);
				internal_scope_guard lock;
				if (text != cat_->text)
				{
...
#ifdef __cpp_char8_t
			cat_proxy& cat_proxy::text(std::u8string s)
			{
				auto text = nana::detail::to_nstring(s);
				internal_scope_guard lock;
				if (text != cat_->text)
...
#ifdef __cpp_char8_t
			void cat_proxy::push_back(std::u8string_view s)
			{
				push_back(to_string(s));
			}
#endif
	...
	//class listbox
		...
		listbox::size_type listbox::append_header(std::string s, unsigned width)
		{
			internal_scope_guard lock;
			auto & ess = _m_ess();
			auto pos = ess.header.create(&ess, nana::detail::to_nstring(std::move(s)), width);
			ess.update();
            return pos;
		}
		...
		listbox::cat_proxy listbox::append(std::string s)
		{
			internal_scope_guard lock;
			auto & ess = _m_ess();
			auto new_cat_ptr = ess.lister.create_category(nana::detail::to_nstring(std::move(s)));
			ess.update();
		...
		void listbox::append(std::initializer_list<std::string> categories)
		{
			...
			for (auto & arg : categories)
				ess.lister.create_category(native_string_type(nana::detail::to_nstring(arg)));
			ess.update();
		}
...
#ifdef __cpp_char8_t
		listbox::size_type listbox::append_header(std::u8string text, unsigned width)
		{
			return append_header(to_string(text), width);
		}

		listbox::cat_proxy listbox::append(std::u8string category)
		{
			return append(to_string(category));
		}

		void listbox::append(std::initializer_list<std::u8string> categories)
		{
			internal_scope_guard lock;
			auto & ess = _m_ess();

			for (auto & c : categories)
				ess.lister.create_category(nana::detail::to_nstring(c));
			ess.update();			
		}		
#endif
		...
		auto listbox::insert(cat_proxy cat, std::string str) -> cat_proxy
		{
			internal_scope_guard lock;
			auto & ess = _m_ess();
			auto new_cat_ptr = ess.lister.create_category(nana::detail::to_nstring(std::move(str)), cat.position());
			return cat_proxy{ &ess, new_cat_ptr };
		}
...
#ifdef __cpp_char8_t
		auto listbox::insert(cat_proxy cat, std::u8string_view str) -> cat_proxy
		{
			return insert(cat, nana::detail::to_nstring(str));
		}
#endif
...
#ifdef __cpp_char8_t
		void listbox::insert_item(const index_pair& pos, std::u8string_view text)
		{
			insert_item(pos, to_string(text));
		}
#endif
		...
		void listbox::set_sort_compare(size_type col, std::function<bool(const std::string&, std::any*, const std::string&, std::any*, bool reverse)> strick_ordering)
		{
			internal_scope_guard lock;
